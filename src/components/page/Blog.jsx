...
`,
  ...
  const [first, setfirst] = useState(contactCar);
    ...
    <div className="flex flex-col gap-28">
      ...
      <div className="h-96 relative text-white ">
        ...
        <div className="w-11/12 absolute top-0 left-0 right-0 flex h-full justify-center flex-col gap-2 mx-auto">
          ...
          <div className="font-semibold text-3xl">Blog</div>
        </div>
      </div>
      {/* banner ends here */}

      <div className="grid grid-cols-4 gap-5 gap-y-9 w-11/12 mx-auto">
        {data.map((val, i) => {
          return (
            <Link key={i} to={`/blog/${JSON.stringify(val.title)}`} state={val}>
              <div
                ...
                className="shadow-md border group flex flex-col gap-5 hover:scale-110  transition-all duration-700 delay-75 ease-in-out p-3 py-5 rounded-2xl"
              >
                <div className="h-44 w-full rounded-2xl bg-gray-300 overflow-clip">
                  <img
                    src={val.image}
                ...
                <div className="flex flex-col gap-4">
                  <h1
                    className="text-lg group-hover:text-primarycolor group-hover:scale-95 transition-all duration-700 delay-75 ease-in-out  font-medium line-clamp-2"
                    key={val.title}
                  >
                    {val.title}
                  </h1>
                  <div className="line-clamp-3 text-gray-500  text-sm  group-hover:scale-95 transition-all duration-700 delay-75 ease-in-out ">
                    {val.description}
                  </div>
                  <div className="flex text-sm font-light justify-between mt-4 group-hover:scale-95 transition-all duration-700 delay-75 ease-in-out ">
